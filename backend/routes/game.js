...
router.post("/:id/check", async (request, response) => {
  ...
  if (isEndofRound) {
    ...
    if (round_winner > 0) {
      ...
      io.to(game_socket_id).emit('showPopup', { message });
  ...
  io.to(game_socket_id).emit(GAME_CONSTANTS.STATE_UPDATED, {
    gameId,
    flopCards,
    turnCards,
    riverCards,
...
// Call this function to test out certain combinations of hands
const testHandRank = () => {
  const test = 
  {"userId": [
        1,
